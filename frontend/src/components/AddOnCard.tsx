...
const AddOnCard = ({ item, compact = false }: AddOnCardProps) => {
    ...
    const handleAdd = () => {
        if (!isInCart) {
            ...
            const result = addToCart(cartItemData);

            if (!result.success) {
                setPendingItem(cartItemData);
    ...
    if (compact) {
        return (
            <>
                ...
                <ConflictModal isOpen={conflictModalOpen} onClose={() => { setConflictModalOpen(false); setPendingItem(null); }} onConfirmClear={handleConfirmClear} />
            </>
        );
    }
